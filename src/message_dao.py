...
class MessageDao(object):
    ...
    def _initialize_tables(self) -> None:
        ...
        with self.connection.cursor() as cursor:
            ...
            cursor.execute(
                ...
                "reduced_subject VARCHAR(255) NOT NULL,"
    ...
    def store(self, message: Message) -> None:
        # Use only the portion of the subject that comes after the brackets that indicate patch information
        reduced_subject = message.subject.partition("] ")[2].lower()
        link = lore_link(message.id)
        sql = "REPLACE INTO Messages VALUES (%s, %s, %s, %s, %s, %s, %s)"
        with self.connection.cursor() as cursor:
            cursor.execute(sql, (message.id, reduced_subject, message.from_,
    ...
    def get(self, message_id: str) -> Optional[Message]:
        sql = "SELECT archive_hash, change_id FROM Messages WHERE message_id=%s"
        with self.connection.cursor() as cursor:
            cursor.execute(sql, (message_id,))
            res = cursor.fetchone()
        if res is None:
